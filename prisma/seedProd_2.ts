...
async function main() {
  ...
  for (let i = 0; i < dfAuth.shape[0]; i += 1) {
    ...
    await prisma.respondents.update({
      ...
      data: {
        ...
        file_path_list_int: respondentData[0].file_path_list_int,
  ...
  const {
    ...
    error: similarityItemSelectError,
  } = await supabaseDev
    .from("SimilarityItem")
    .select("*");
  if (similarityItemSelectError) {
    console.error(similarityItemSelectError);
    process.exit(1);
  }
  await prisma.similarityItem.createMany({
    data: similarityItemList.map(({ id, ...rest }) => rest),
    skipDuplicates: true,
  });
